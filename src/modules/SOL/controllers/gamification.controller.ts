...
export class GamificationController {
  ...
  @ApiResponse({ status: 200, description: 'All gamification data retrieved successfully' })
  ...
  async getGamification(@Param('userId') userId: string): Promise<Gamification[]> {
  ...
  @Post('add-points')
  @ApiOperation({ summary: 'Add points to a user\'s gamification profile along with rewards' })
  @ApiBody({ type: AddPointsDto })
  @ApiResponse({ status: 201, description: 'Points and rewards added successfully' })
  @ApiResponse({ status: 400, description: 'Invalid data' })
  ...
  @ApiResponse({ status: 200, description: 'User ranking retrieved successfully' })

...
export class ProposalRepository {
  ...
  async updateProposedWin(
    _id: string,
    dto: ProposalWinRequestDto,
  ): Promise<ProposalModel> {
    return await this._model.findOneAndUpdate(
      { _id },
      {
  ...
  async acceptForFornecedorProposal(
    _id: string,
    dto: ProposalAcceptedRequestDto,
  ): Promise<ProposalModel> {
    return await this._model.findOneAndUpdate(
      { _id },
      {
  ...
  async acceptForRevisorProposal(
    _id: string,
    dto: ProposalAcceptedRequestDto,
  ): Promise<ProposalModel> {
    return await this._model.findOneAndUpdate(
  ...
  async listByBid(bidId: string): Promise<ProposalModel[]> {
    const proposals = await this._model
      .find({ bid: { _id: bidId } })
      .populate("proposedBy")
  ...
  async listByUser(proposedById: string): Promise<ProposalModel[]> {
    ...
    const supplier_user = await this._userRepository.getUserBySupplierId(
      proposedByUser.supplier.id,
    );
    ...
    const proposals = await this._model
      .find({ proposedBy: { $in: supplierIds } })
      .populate("proposedBy")
      .populate("allotment");
  ...
  async listByBidsWaiting(bidId: string): Promise<ProposalModel[]> {
    ...
    if (proposals.length > 1) {
      ...
      for (let iterator of sortedProposals) {
        if (
          iterator.status === ProposalStatusEnum["aguardando1"] ||
          iterator.status === ProposalStatusEnum["aguardando2"]
        ) {
  ...
  async getProposalWin(bidId: string): Promise<ProposalModel> {
    ...
    const sortedProposals = list
      ...
      .filter(
        (el) =>
          el.status === ProposalStatusEnum["aguardando1"] ||
          el.status === ProposalStatusEnum["aguardando2"],
      );
  ...
  async listProposalByBidTie(bidId: string): Promise<ProposalModel[]> {
    return await this._model
      .find({ bid: { _id: bidId }, proposalWin: true })
      .populate("proposedBy")
      .populate("refusedBy")
      .populate("acceptedFornecedor")
      .populate("acceptedRevisor")

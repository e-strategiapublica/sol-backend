...
export class NotificationService {
  ...
  async register(dto: NotificationRegisterDto): Promise<NotificationModel> {
    ...
    if (!result)
      throw new BadRequestException(
        "Não foi possivel cadastrar essa notificação!",
      );

    return result;
  }

  async registerByBidCreation(
    _id: string,
    dto: NotificationRegisterDto,
  ): Promise<UserModel[] | void> {
    const user: UserModel[] =
      await this._userRepository.getUserBySupplierId(_id);
  ...
  async registerforAssociationCreation(
    _id: string,
    dto: NotificationRegisterDto,
  ): Promise<UserModel> {
    ...
    if (!user) {
      throw new BadRequestException(
        "Usuário não encontrado, não foi possivel cadastrar essa notificação!",
      );
    }
  ...
  async updateTittle(
    ...
    const result = await this._notificationRepository.updateTitleDescription(
      _id,
      dto,
    );
    return result;
  ...
  async registerForRealese(
    ...
    const associationUser =
      await this._userRepository.getById(associationUserId);
    ...
    const resultReviewer =
      await this._notificationRepository.register(dtoReviewer);
    const resultAssociation =
      await this._notificationRepository.register(dtoAssociation);
    ...
    await this._userRepository.updateNotifications(
      associationUser.id,
      resultAssociation,
    );
  }

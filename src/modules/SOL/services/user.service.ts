...
export class UserService {
  ...
  async register(
    ...
    if (dto.document) {
      const userByDocument = await this._userRepository.getByDocument(
        dto.document,
      );
      if (userByDocument) {
    ...
    if (dto.association) {
      const userByAssociation = await this._associationService.getById(
        dto.association,
      );
    ...
    return new UserRegisterResponseDto(result.id, result.email);
  }

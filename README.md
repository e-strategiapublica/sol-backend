...
Para rodar todos os testes unitários, use o comando:

```bash
yarn test --config jest.config.js
```

Isso executa os testes de todas as suítes.

Para rodar um teste específico, informe o caminho do arquivo de teste:

```bash
yarn test --config jest.config.js src/modules/SOL/services/authentication.service.test.ts
```
